...
**每个好友还能单独设一套**——设置页往下拉一格「按好友设置」（或者点首页会话那一行最右边的小按钮），
填上 TA 的会话名，单独定关系 / 口吻 / 参考条数；存下以后跟 TA 聊自动就是这一套，
没填的项仍然跟着全局走。
...
3. 顺手选一下「你们的关系」（恋人 / 朋友 / 同事 / 家人 / 自定义）。保存，可以用了。
   想给某个好友单独定口吻，就再往下拉一格「按好友设置」：填 TA 的会话名，存一套只给 TA 用的。
...
改完点「保存设置」，下一次生成 / 润色立即生效，不用重启。「按好友设置」是**按人存的**，
点它自己的「保存这个好友」，跟全局的「保存设置」互不影响。
...
| 按好友设置 | 给单个好友 / 群单独定一套「关系 + 说话风格 + 参考上下文」，存下以后跟 TA 聊自动用它；留空 / 选「跟随全局」的项跟全局 | `config.json` → `friends`（`{"会话名": {"relationship": …, "style": …, "context": …}}`） |
...
只在内存里，不落盘，重启回默认。**「按好友设置」不在此列**——它跟上面那张表一样落在 `config.json` 里，重启还在。
