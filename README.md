...
**每个好友还能单独设一套**——点首页会话那一行最右边的小按钮，进的是**单独一页**「单个好友设置」
（不跟全局设置挤在一起）：填上 TA 的会话名，单独定关系 / 口吻 / 参考条数；存下以后跟 TA 聊自动就是这一套，
没填的项仍然跟着全局走。
...
<tr>
<td width="50%"><img src="docs/ui_friend.png" alt="单个好友设置"></td>
<td align="center">「单个好友设置」是**单独一页**（首页会话那一行最右边的小按钮进），只有三项：关系 / 口吻、说话风格、参考上下文条数；没填的项跟全局走</td>
</tr>
</table>
...
3. 顺手选一下「你们的关系」（恋人 / 朋友 / 同事 / 家人 / 自定义）。保存，可以用了。
   想给某个好友单独定口吻，不用在设置页里翻——点首页会话那一行最右边的小按钮，
   进「单个好友设置」那一页，填 TA 的会话名，存一套只给 TA 用的。
...
- 喂口吻样本：把你自己最近 12 条短消息原样给它，照着你的用词、句长、标点习惯写；设置里的
  「说话风格」再补一句你自己的描述（给单个好友单独设过的那套优先）
...
> 嫌碍眼就把输入区最下面那行的采集开关拨到「已暂停」，黄框立刻消失。
...
改完点「保存设置」，下一次生成 / 润色立即生效，不用重启。「单个好友设置」是**单独一页**（首页那个小按钮进），
按人存的，点它自己的「保存这个好友」，跟全局的「保存设置」互不影响。
...
| 单个好友设置 | **单独一页**：首页会话那一行最右边的小按钮进去，给单个好友 / 群单独定一套「关系 + 说话风格 + 参考上下文」，存下以后跟 TA 聊自动用它；留空 / 选「跟随全局」的项跟全局 | `config.json` → `friends`（`{"会话名": {"relationship": …, "style": …, "context": …}}`） |
...
主界面上那几个（输入区最下面那行的「自动生成」和采集开关、标题栏的图钉、「当前会话」和「回复对象」下拉、「带 @」勾选框、输入框内容）
只在内存里，不落盘，重启回默认。**「单个好友设置」不在此列**——它跟上面那张表一样落在 `config.json` 里，重启还在。

会话名就是微信顶部那个标题：**一个字都不能差**（包括群名），改过名字的老设置不会跟着走。
