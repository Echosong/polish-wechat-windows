...
这里不认识任何「来源」——来源表在 core/providers.py。出错一律转成 ChatError，消息过脱敏。
...
try:  # 当模块导入 / 当脚本直接跑 都能用
    from .keys import ChatError, fail
except ImportError:
    from keys import ChatError, fail
...
def list_models(protocol: str, base_url: str | None, api_key: str,
    ...
    """某个地址上能用的模型 id，去重排序。失败抛 ChatError，消息直接显示在设置页上。"""
    if protocol == "anthropic":
        import anthropic
...
if __name__ == "__main__":
    ...
    chat("openai", "https://opencode.ai/zen/go/v1", "k", "deepseek-v4.1-flash", "S", ["U"],
         headers={"x-opencode-session": "sid", "User-Agent": "polish-chat"})
    assert seen["openai.init"]["default_headers"] == {
        "x-opencode-session": "sid", "User-Agent": "polish-chat"}
    ...
    assert "default_headers" not in seen["openai.init"]
    list_models("openai", "https://x/v1", "k", headers={"User-Agent": "polish-chat"})
    assert seen["openai.init"]["default_headers"] == {"User-Agent": "polish-chat"}
    assert list_models("anthropic", "", "k") == ["claude-x", "claude-y"]
    assert list_models("gemini", "", "k") == ["gemini-1", "gemini-2"]

    # 出错 → 一句人话的 ChatError，带上状态码，不泄露 key
