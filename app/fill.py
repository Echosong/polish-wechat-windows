# -*- coding: utf-8 -*-
"""把文字放进微信输入框：写剪贴板 → 点输入框 → Ctrl+V。

两个出口，差别只有最后那一下：
- fill()  只填，不发。发不发、改不改，人来。
- send()  填完按一次回车。只在用户自己点了「发送」或「润色并发送」（含 Ctrl+回车 /
          Ctrl+Shift+回车）时才走到这里——程序没有任何自动触发路径（不监听新消息、不轮询、
          不代按）。「润色并发送」多出来的只有发送前那一次润色调用，发起人仍然是用户自己。
"""
import ctypes
...
def fill(hwnd, area, text):
    ...
    # fill() 到此为止。发不发、改不改，人来。


def send(hwnd, area, text):
    """填进输入框再按一次回车。只由用户点「发送」/「润色并发送」触发，没有别的调用方。"""
    fill(hwnd, area, text)
    # 微信把粘贴的内容渲染进输入框要几十毫秒，立刻回车偶尔只发出半截
    time.sleep(0.25)
    u32.keybd_event(VK_RETURN, 0, 0, 0)
    u32.keybd_event(VK_RETURN, 0, 2, 0)
