...
def window_rect(hwnd):
    """窗口在屏幕上的矩形（物理像素）(left, top, right, bottom)；最小化或取不到就返回 None。
    悬浮框靠它贴在聊天窗口旁边。"""
    if not hwnd or u32.IsIconic(hwnd):
        return None
    rect = w.RECT()
    if ctypes.windll.dwmapi.DwmGetWindowAttribute(hwnd, 9, ctypes.byref(rect),
                                                  ctypes.sizeof(rect)) != 0:
        if not u32.GetWindowRect(hwnd, ctypes.byref(rect)):
            return None
    if rect.right - rect.left < 200 or rect.bottom - rect.top < 200:  # 图标态/异常值，别拿它定位
        return None
